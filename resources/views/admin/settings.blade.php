        ...
        <div class="card-body">
            <form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data" class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="logo_url" value="{{ old('logo_url', $siteSetting->logo_url) }}" class="form-control @error('logo_url') is-invalid @enderror" placeholder="https://... hoặc để trống nếu upload file">
                    @error('logo_url')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    <label class="form-label">Upload logo từ máy tính</label>
                    <input type="file" name="logo_file" accept=".jpg,.jpeg,.png,.webp,.svg" class="form-control @error('logo_file') is-invalid @enderror">
                    @error('logo_file')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <div class="form-text">Ưu tiên file PNG/SVG nền trong suốt. Dung lượng tối đa 2MB.</div>
                </div>

                @if (!empty($siteSetting->logo_url))
                    <div class="col-12">
                        <label class="form-label d-block">Logo hiện tại</label>
                        <img src="{{ $siteSetting->logo_url }}" alt="Logo hiện tại" style="max-height: 60px; width: auto;" class="bg-light p-2 border rounded">
                    </div>
                @endif
